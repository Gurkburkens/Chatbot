const USD_SEK = 10.5, RSI_KÖP = 38, RSI_SÄLJ = 63, ADX_GRÄNS = 25, TRAILING = 0.15, MIN_VINST = 0.03, MAX_PTS = 80;
let liveSymbol = 'BTCUSDT', liveHistory = [], killSwitch = false, bigChart = null, calcChart = null;
...
function initBigChart() {
    bigChart = new Chart(document.getElementById('bigChart'), {
        type: 'line',
        data: {
            labels: Array(MAX_PTS).fill(''), datasets: [
...
function setMetric(id, value, cls) {
    ...
    el.textContent = value; el.className = 'metric-tile-value ' + cls;
}

function setBadge(id, text, cls) {
    const el = document.getElementById(id); if (!el) return;
    el.textContent = text; el.className = 'ind-badge ' + cls;
}
...
async function uppdateraLive() {
    try {
        const [ticker, klines] = await Promise.all([
            fetch(`https://api.binance.com/api/v3/ticker/price?symbol=${liveSymbol}`).then(r => r.json()),
            fetch(`https://api.binance.com/api/v3/klines?symbol=${liveSymbol}&interval=1h&limit=100`).then(r => r.json()),
        ]);
        const pris = parseFloat(ticker.price);
        const closes = klines.map(k => parseFloat(k[4]));
        liveHistory = [...closes, pris];
        ...
        if (klines.length >= 24) { const p24 = parseFloat(klines[klines.length - 24][4]), ch = (pris - p24) / p24 * 100; const el = document.getElementById('live-change'); el.textContent = (ch >= 0 ? '+' : '') + ch.toFixed(2) + '% (24h)'; el.className = 'price-change ' + (ch >= 0 ? 'pos' : 'neg'); }
    ...
    } catch (e) { console.error('Live-fel:', e); }
}
...
async function uppdateraHeroPriser() {
    try {
        ...
        document.getElementById('hero-btc').textContent = '$' + Math.round(parseFloat(btc.price)).toLocaleString('sv-SE');
        document.getElementById('hero-eth').textContent = '$' + Math.round(parseFloat(eth.price)).toLocaleString('sv-SE');
    } catch (e) { }
}
...
async function körKalkylator() {
    ...
    btn.textContent = 'Beräknar...'; btn.disabled = true;
    try {
        const raw = await fetch(`https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=1d&limit=${dagar + 60}`).then(r => r.json());
        const data = raw.slice(-dagar).map(c => ({ datum: new Date(c[0]).toISOString().slice(0, 10), close: parseFloat(c[4]) }));
        const priser = data.map(d => d.close);
        let cash = kapUSD, coin = 0, entry = 0, topp = 0, totInsatt = kapUSD, totInsattSEK = kapSEK;
        const trades = [], botVals = [], hodlVals = [], insattVals = [];
        const hodlCoin = kapUSD / priser[0];
        let dcaDag = 0, manadDag = 0;
        for (let i = 20; i < priser.length; i++) {
            const p = priser[i], hist = priser.slice(0, i + 1);
            const r = beräknaRSI(hist), ma20 = beräknaMA(hist, 20), avv = (p - ma20) / ma20;
            manadDag++;
            if (manadDag >= 30 && manUSD > 0) { cash += manUSD; totInsatt += manUSD; totInsattSEK += manSEK; manadDag = 0; }
            if (coin > 0) { if (p > topp) topp = p; if (p <= topp * (1 - TRAILING)) { const vp = (p - entry) / entry * 100; trades.push({ datum: data[i].datum, typ: 'SÄLJ', pris: p, vp, orsak: `Trailing stop (${vp >= 0 ? '+' : ''}${vp.toFixed(1)}%)` }); cash = coin * p; coin = 0; entry = 0; topp = 0; } }
            if (coin > 0 && r > RSI_SÄLJ) { const vp = (p - entry) / entry * 100; if (vp >= MIN_VINST * 100) { trades.push({ datum: data[i].datum, typ: 'SÄLJ', pris: p, vp, orsak: `RSI ${r.toFixed(0)}` }); cash = coin * p; coin = 0; entry = 0; topp = 0; } }
            dcaDag++;
            if (dcaDag >= 7) { const bel = Math.min(cash * 0.25, kapUSD * 0.3); if (bel > 3 && r <= RSI_SÄLJ) { const m = bel / p; entry = coin === 0 ? p : (entry * coin + p * m) / (coin + m); if (coin === 0 || p > topp) topp = p; coin += m; cash -= bel; trades.push({ datum: data[i].datum, typ: 'DCA', pris: p, vp: null, orsak: `DCA (RSI ${r.toFixed(0)})` }); dcaDag = 0; } }
            if (avv <= -0.05 && r < RSI_KÖP && cash > 5) { const bel = cash * 0.5, m = bel / p; entry = coin === 0 ? p : (entry * coin + p * m) / (coin + m); if (coin === 0 || p > topp) topp = p; coin += m; cash -= bel; trades.push({ datum: data[i].datum, typ: 'KÖP', pris: p, vp: null, orsak: `Dipp ${(avv * 100).toFixed(1)}%` }); }
            botVals.push(Math.round((cash + coin * p) * USD_SEK));
            hodlVals.push(Math.round((hodlCoin * p + manUSD * Math.floor(i / 30)) * USD_SEK));
            insattVals.push(Math.round(totInsattSEK));
        }
        const labels = data.slice(20).map(d => d.datum);
        const slutBot = botVals[botVals.length - 1], slutHodl = hodlVals[hodlVals.length - 1];
        const botPnl = slutBot - totInsattSEK, botPct = botPnl / totInsattSEK * 100;
        const hodlPct = (slutHodl - totInsattSEK) / totInsattSEK * 100, bankPct = dagar / 365 * 2.5;
        document.getElementById('result-big').innerHTML = `
      <div class="result-card highlight"><div class="result-label">SLUTVÄRDE (BOT)</div><div class="result-value gold">${Math.round(slutBot).toLocaleString('sv-SE')} kr</div><div class="result-sub">Insatt: ${Math.round(totInsattSEK).toLocaleString('sv-SE')} kr</div></div>
      <div class="result-card ${botPct >= 0 ? 'green-bg' : 'red-bg'}"><div class="result-label">BOTENS AVKASTNING</div><div class="result-value ${botPct >= 0 ? 'green' : 'red'}">${botPct >= 0 ? '+' : ''}${botPct.toFixed(1)}%</div><div class="result-sub">${botPnl >= 0 ? '+' : ''}${Math.round(botPnl).toLocaleString('sv-SE')} kr</div></div>
      <div class="result-card blue-bg"><div class="result-label">HODL HADE GETT</div><div class="result-value blue">${hodlPct >= 0 ? '+' : ''}${hodlPct.toFixed(1)}%</div><div class="result-sub">${Math.round(slutHodl).toLocaleString('sv-SE')} kr</div></div>
      <div class="result-card"><div class="result-label">SPARKONTO (2.5%/år)</div><div class="result-value muted">+${bankPct.toFixed(2)}%</div><div class="result-sub">Bot: ${botPct >= bankPct ? '+' : ''}${(botPct - bankPct).toFixed(1)}%</div></div>
      <div class="result-card"><div class="result-label">ANTAL AFFÄRER</div><div class="result-value gold">${trades.length}</div><div class="result-sub">${trades.filter(t => t.typ === 'SÄLJ').length} sälj · ${trades.filter(t => t.typ !== 'SÄLJ').length} köp</div></div>
      <div class="result-card"><div class="result-label">INSATT TOTALT</div><div class="result-value" style="color:var(--text)">${Math.round(totInsattSEK).toLocaleString('sv-SE')} kr</div><div class="result-sub">${kapSEK.toLocaleString('sv-SE')} kr start${manSEK > 0 ? ' + ' + manSEK + ' kr/mån' : ''}</div></div>`;
        if (calcChart) calcChart.destroy();
        calcChart = new Chart(document.getElementById('calcChart'), {
            type: 'line', data: {
                labels, datasets: [
                    { label: 'Bot', data: botVals, borderColor: '#f59e0b', borderWidth: 2, pointRadius: 0, tension: 0.3, fill: true, backgroundColor: 'rgba(245,158,11,0.06)' },
                    { label: 'HODL', data: hodlVals, borderColor: '#60a5fa', borderWidth: 1.5, borderDash: [5, 4], pointRadius: 0, tension: 0.3, fill: false },
                    { label: 'Insatt', data: insattVals, borderColor: '#3a3a50', borderWidth: 1, borderDash: [2, 3], pointRadius: 0, fill: false },
                ]
            }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { backgroundColor: '#13131a', borderColor: '#22222e', borderWidth: 1, titleColor: '#6b6b88', bodyColor: '#f0f0f8', callbacks: { label: ctx => `${ctx.dataset.label}: ${Math.round(ctx.parsed.y).toLocaleString('sv-SE')} kr` } } }, scales: { x: { ticks: { color: '#3a3a50', font: { size: 10 }, maxTicksLimit: 8 }, grid: { color: '#1a1a24' } }, y: { ticks: { color: '#3a3a50', font: { size: 10 }, callback: v => Math.round(v).toLocaleString('sv-SE') + ' kr' }, grid: { color: '#1a1a24' } } } }
        });
        document.getElementById('trades-count-label').textContent = `${trades.length} affärer totalt`;
        const mini = document.getElementById('trades-mini');
        mini.innerHTML = '<div class="trade-row-mini" style="color:var(--muted);font-size:10px;"><span>DATUM</span><span>TYP</span><span>PRIS</span><span>VINST</span><span>ORSAK</span></div>' +
            trades.slice(-40).reverse().map(t => `<div class="trade-row-mini"><span>${t.datum}</span><span><span class="trade-tag ${t.typ === 'SÄLJ' ? 'tag-sell' : t.typ === 'DCA' ? 'tag-dca' : 'tag-buy'}">${t.typ}</span></span><span>$${Math.round(t.pris).toLocaleString()}</span><span style="color:${t.vp === null ? 'var(--muted)' : t.vp >= 0 ? 'var(--green)' : 'var(--red)'}">${t.vp === null ? '–' : (t.vp >= 0 ? '+' : '') + t.vp.toFixed(1) + '%'}</span><span style="color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${t.orsak}</span></div>`).join('');
        const res = document.getElementById('calc-results'); res.style.display = 'block'; res.classList.add('fade-in'); res.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    } catch (e) { alert('Fel: ' + e.message); }
    finally { btn.textContent = 'Beräkna vad boten hade genererat'; btn.disabled = false; }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    uppdateraHeroPriser();
    uppdateraLive();
    setInterval(uppdateraLive, 20000);
    setInterval(uppdateraHeroPriser, 30000);
    setTimeout(körKalkylator, 600);
});